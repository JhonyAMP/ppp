...
<div>
    ...
    @include('livewire.admin.modal.config_date')
                ...
                <table class="w-full text-left table-auto min-w-max">
                    <x-table-thead>
                        <tr>
                            ...
                            <th class="p-3 font-normal text-center">Empresa</th>
                            ...
                            <th class="py-3 px-8 font-normal text-center">Acciones</th>
                        </tr>
                    </x-table-thead>
                    <tbody class="text-sm divide-y divide-gray-300">
                        @foreach ($users as $user)
                            <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                ...
                                <td class="p-3">{{ $user->razon_social }}</td>
                                ...
                                <td class="p-3 w-16">
                                    <div class="flex justify-center relative">
                                        ...
                                        @can('admin.supervisor-ppp')
                                            <x-button-tooltip hover="gray"
                                                content="{{ $user->supervisor_id ? 'Editar Supervisor' : 'Asignar Supervisor' }}"
                                                wire:click="mostrarSupervisores({{ $user->id }}, {{ $user->company_id }})">
                                                <i class="fa-solid fa-user-lock fa-fw"></i>
                                            </x-button-tooltip>
                                        @endcan
                                        @can('admin.evaluation.ppp')
                                            <x-button-tooltip hover="gray"
                                                content="Configurar"
                                                wire:click="configDates({{ $user->estudiante_id }})">
                                                <i class="fa-regular fa-clock fa-fw"></i>
                                            </x-button-tooltip>
                                        @endcan
                                        @can('admin.evaluation.ppp')
                                            <x-button-tooltip hover="gray"
                                                content="Evaluar"
                                                wire:click="evaluar({{ $user->estudiante_id }})">
